(this.webpackJsonpfront_end_take_home =
  this.webpackJsonpfront_end_take_home || []).push([
  [0],
  {
    120: function(e, t, a) {
      "use strict";
      a.r(t);
      var n = a(0),
        r = a.n(n),
        c = a(10),
        i = a.n(c),
        l = (a(97), a(181)),
        s = a(78),
        o = Object(s.a)({}),
        m = a(35),
        u = a(12),
        g = a(11),
        E = a(161),
        p = a(187),
        d = a(182),
        b = a(188),
        f = a(189),
        h = a(169),
        A = a(170),
        v = a(165),
        j = a(122),
        O = a(166),
        S = a(163),
        y = Object(E.a)(function(e) {
          return {
            paper: {
              maxWidth: 1e3,
              margin: "".concat(e.spacing(3), "px auto"),
              padding: e.spacing(5)
            },
            header: { backgroundColor: "lightblue" },
            headingText: { verticalAlign: "middle" },
            instructions: { backgroundColor: "" },
            instructionsText: { verticalAlign: "middle" },
            divider: { backgroundColor: "grey", width: "100%" },
            gridContainer: { margin: "0 auto" },
            select: { width: "10rem" }
          };
        }),
        w = function() {
          var e = y(),
            t = Object(u.f)(),
            a = Object(n.useState)(10),
            c = Object(g.a)(a, 2),
            i = c[0],
            l = c[1],
            s = Object(n.useState)(""),
            o = Object(g.a)(s, 2),
            m = o[0],
            E = o[1],
            w = Object(n.useState)(""),
            x = Object(g.a)(w, 2),
            P = x[0],
            C = x[1];
          return r.a.createElement(
            "div",
            null,
            r.a.createElement(
              S.a,
              { className: e.paper },
              r.a.createElement(
                v.a,
                {
                  container: !0,
                  spacing: 2,
                  direction: "row",
                  alignItems: "center",
                  justify: "center",
                  alignContent: "center",
                  className: e.gridContainer
                },
                r.a.createElement(
                  v.a,
                  {
                    container: !0,
                    justify: "center",
                    xs: 12,
                    item: !0,
                    className: e.header
                  },
                  r.a.createElement(
                    j.a,
                    {
                      variant: "h3",
                      display: "inline",
                      className: e.headingText
                    },
                    "Teikametrics Quiz"
                  )
                ),
                r.a.createElement(
                  v.a,
                  {
                    container: !0,
                    justify: "center",
                    xs: 12,
                    item: !0,
                    className: e.instructions
                  },
                  r.a.createElement(
                    v.a,
                    { container: !0, justify: "center", xs: 12, item: !0 },
                    r.a.createElement(
                      j.a,
                      {
                        variant: "h4",
                        display: "inline",
                        className: e.instructionsText
                      },
                      "Instructions"
                    )
                  ),
                  r.a.createElement(
                    v.a,
                    { container: !0, justify: "center", xs: 10, item: !0 },
                    r.a.createElement(
                      j.a,
                      {
                        variant: "h6",
                        display: "inline",
                        className: e.instructionsText
                      },
                      r.a.createElement(
                        "ul",
                        null,
                        r.a.createElement(
                          "li",
                          null,
                          "Once the test has started don't press back/refresh button."
                        ),
                        r.a.createElement(
                          "li",
                          null,
                          "Don't try to conduct any fraud practices."
                        )
                      )
                    )
                  )
                ),
                r.a.createElement(O.a, { className: e.divider }),
                r.a.createElement(
                  v.a,
                  { xs: 12, sm: 3, item: !0 },
                  r.a.createElement(p.a, {
                    id: "standard-number",
                    label: "Number of questions",
                    type: "number",
                    value: i,
                    onChange: function(e) {
                      return l(parseInt(e.target.value));
                    }
                  })
                ),
                r.a.createElement(
                  v.a,
                  { xs: 12, sm: 3, item: !0 },
                  r.a.createElement(
                    h.a,
                    null,
                    r.a.createElement(
                      b.a,
                      { id: "demo-simple-select-label" },
                      "Difficulty Level"
                    ),
                    r.a.createElement(
                      d.a,
                      {
                        className: e.select,
                        labelId: "demo-simple-select-label",
                        id: "demo-simple-select",
                        value: m,
                        onChange: function(e) {
                          E(e.target.value);
                        }
                      },
                      r.a.createElement(f.a, { value: "easy" }, "Easy"),
                      r.a.createElement(f.a, { value: "medium" }, "Medium"),
                      r.a.createElement(f.a, { value: "hard" }, "Hard")
                    )
                  )
                ),
                r.a.createElement(
                  v.a,
                  { xs: 12, sm: 3, item: !0 },
                  r.a.createElement(
                    h.a,
                    null,
                    r.a.createElement(
                      b.a,
                      { id: "demo-simple-select-label" },
                      "Category"
                    ),
                    r.a.createElement(
                      d.a,
                      {
                        className: e.select,
                        labelId: "demo-simple-select-label",
                        id: "demo-simple-select",
                        value: P,
                        onChange: function(e) {
                          C(e.target.value);
                        }
                      },
                      r.a.createElement(
                        f.a,
                        { value: "10" },
                        "General Knowledge"
                      ),
                      r.a.createElement(f.a, { value: "21" }, "Sports")
                    )
                  )
                ),
                r.a.createElement(
                  v.a,
                  { container: !0, xs: 12, justify: "center", item: !0 },
                  r.a.createElement(
                    A.a,
                    {
                      variant: "contained",
                      color: "primary",
                      onClick: function() {
                        var e = "?numQuestions=" + i.toString();
                        P && (e += "&category=" + P),
                          m && (e += "&difficulty=" + m),
                          t.push("/test".concat(e));
                      }
                    },
                    "Start the Test"
                  )
                )
              )
            )
          );
        },
        x = function() {
          return r.a.createElement("div", null, r.a.createElement(w, null));
        },
        P = a(184),
        C = a(174),
        J = a(74),
        Q = a(75),
        I = a.n(Q),
        B = new (function e() {
          Object(J.a)(this, e),
            (this.getQuestions = function() {
              var e =
                  arguments.length > 0 && void 0 !== arguments[0]
                    ? arguments[0]
                    : "10",
                t =
                  arguments.length > 1 && void 0 !== arguments[1]
                    ? arguments[1]
                    : "",
                a =
                  arguments.length > 2 && void 0 !== arguments[2]
                    ? arguments[2]
                    : "",
                n = "?amount=" + e;
              return (
                t && (n += "&category=" + t),
                a && (n += "&difficulty=" + a),
                I.a.get("https://opentdb.com/api.php".concat(n))
              );
            });
        })(),
        k = a(185),
        N = a(186),
        D = a(171),
        T = function(e) {
          var t = document.createElement("textarea");
          return (t.innerHTML = e), t.value;
        },
        V = Object(E.a)(function(e) {
          return {
            paper: {
              maxWidth: 1e3,
              margin: "".concat(e.spacing(3), "px auto"),
              padding: e.spacing(5)
            },
            question: {
              backgroundColor: "lightgreen",
              color: "black",
              verticalAlign: "middle",
              height: "4rem"
            },
            option: { backgroundColor: "white" }
          };
        }),
        R = function(e) {
          var t = e.details,
            a = e.handleSelection,
            n = V();
          return r.a.createElement(
            "div",
            null,
            t
              ? r.a.createElement(
                  r.a.Fragment,
                  null,
                  r.a.createElement(
                    v.a,
                    {
                      container: !0,
                      justify: "center",
                      alignItems: "center",
                      xs: 12,
                      item: !0,
                      className: n.question
                    },
                    T(t.question)
                  ),
                  r.a.createElement(
                    v.a,
                    {
                      container: !0,
                      justify: "center",
                      xs: 12,
                      item: !0,
                      className: n.option
                    },
                    r.a.createElement(
                      N.a,
                      {
                        "aria-label": "options",
                        name: "options1",
                        onChange: a
                      },
                      r.a.createElement(D.a, {
                        value: T(t.correct_answer),
                        control: r.a.createElement(k.a, null),
                        label: T(t.correct_answer)
                      }),
                      t.incorrect_answers.map(function(e) {
                        return r.a.createElement(D.a, {
                          value: T(e),
                          control: r.a.createElement(k.a, null),
                          label: T(e)
                        });
                      })
                    )
                  )
                )
              : ""
          );
        },
        F = a(173),
        M = a(183),
        q = function(e) {
          return r.a.createElement(
            "div",
            { style: { width: "100%" } },
            r.a.createElement(
              M.a,
              { display: "flex", alignItems: "center" },
              r.a.createElement(
                M.a,
                { width: "100%", mr: 1 },
                r.a.createElement(
                  F.a,
                  Object.assign({ variant: "determinate" }, e)
                )
              ),
              r.a.createElement(
                M.a,
                { minWidth: 35 },
                r.a.createElement(
                  j.a,
                  { variant: "body2", color: "textSecondary" },
                  "".concat(Math.round(e.value), "%")
                )
              )
            )
          );
        },
        G = Object(E.a)(function(e) {
          return {
            paper: {
              maxWidth: 1e3,
              margin: "".concat(e.spacing(3), "px auto"),
              padding: e.spacing(5)
            }
          };
        }),
        X = function() {
          var e = G(),
            t = Object(u.f)(),
            a = Object(n.useState)([]),
            c = Object(g.a)(a, 2),
            i = c[0],
            l = c[1],
            s = Object(n.useState)(!0),
            o = Object(g.a)(s, 2),
            m = o[0],
            E = o[1],
            p = Object(n.useState)(0),
            d = Object(g.a)(p, 2),
            b = d[0],
            f = d[1],
            h = Object(n.useState)(0),
            j = Object(g.a)(h, 2),
            O = j[0],
            y = j[1],
            w = Object(n.useState)(0),
            x = Object(g.a)(w, 2),
            J = x[0],
            Q = x[1],
            I = Object(n.useState)(""),
            k = Object(g.a)(I, 2),
            N = k[0],
            D = k[1],
            T = Object(n.useState)(""),
            V = Object(g.a)(T, 2),
            F = V[0],
            M = V[1],
            X = Object(n.useState)(!1),
            W = Object(g.a)(X, 2),
            H = W[0],
            L = W[1],
            K = Object(n.useState)(0),
            z = Object(g.a)(K, 2),
            Y = z[0],
            Z = z[1],
            U = Object(n.useState)(!1),
            _ = Object(g.a)(U, 2),
            $ = _[0],
            ee = _[1];
          Object(n.useEffect)(function() {
            localStorage.setItem("currentResults", JSON.stringify({}));
            var e = new URLSearchParams(
                window.location.hash.replace("#/test", "")
              ),
              t = e.get("numQuestions"),
              a = e.get("category"),
              n = e.get("difficulty");
            B.getQuestions(t || void 0, a || void 0, n || void 0).then(function(
              e
            ) {
              l(e.data.results),
                y(e.data.results.length - 1),
                E(!1),
                setInterval(function() {
                  return Z(function(e) {
                    return e + 1;
                  });
                }, 1e3),
                f(0),
                D(e.data.results[0].correct_answer);
            });
          }, []),
            Object(n.useEffect)(
              function() {
                b > 0 && i[b] && D(i[b].correct_answer);
              },
              [b]
            ),
            Object(n.useEffect)(
              function() {
                if (H) {
                  t.push("/finish");
                  var e = O + 1,
                    a = {
                      totalQuestions: e,
                      correctAnswers: J,
                      timer: Y,
                      wrongAnswers: e - J,
                      percentageSuccess: Math.round((J / e) * 100)
                    };
                  localStorage.setItem("currentResults", JSON.stringify(a));
                  var n = JSON.parse(
                    localStorage.getItem("resultsHistory") || "[]"
                  );
                  n.push(a),
                    localStorage.setItem("resultsHistory", JSON.stringify(n));
                }
              },
              [H]
            );
          return r.a.createElement(
            "div",
            null,
            m
              ? r.a.createElement(
                  r.a.Fragment,
                  null,
                  r.a.createElement(
                    S.a,
                    { className: e.paper },
                    r.a.createElement(
                      v.a,
                      {
                        spacing: 2,
                        container: !0,
                        xs: 12,
                        item: !0,
                        justify: "center"
                      },
                      r.a.createElement(
                        v.a,
                        { container: !0, xs: 12, item: !0, justify: "center" },
                        r.a.createElement(C.a, null)
                      ),
                      r.a.createElement(
                        v.a,
                        { container: !0, xs: 12, item: !0, justify: "center" },
                        "Loading Questions"
                      )
                    )
                  )
                )
              : r.a.createElement(
                  r.a.Fragment,
                  null,
                  r.a.createElement(
                    S.a,
                    { className: e.paper },
                    r.a.createElement(
                      v.a,
                      {
                        container: !0,
                        direction: "row",
                        spacing: 0,
                        justify: "center",
                        xs: 12
                      },
                      r.a.createElement(q, {
                        value: ((b + 1) / (O + 1)) * 100
                      }),
                      r.a.createElement(
                        v.a,
                        { xs: 12, item: !0 },
                        r.a.createElement(R, {
                          details: i[b],
                          handleSelection: function(e) {
                            M(e.target.value), ee(!1);
                          }
                        })
                      ),
                      r.a.createElement(
                        v.a,
                        { container: !0, justify: "center", xs: 12, item: !0 },
                        r.a.createElement(
                          A.a,
                          {
                            color: "primary",
                            onClick: function() {
                              F
                                ? (f(function(e) {
                                    return e + 1;
                                  }),
                                  N === F &&
                                    Q(function(e) {
                                      return e + 1;
                                    }),
                                  M(""),
                                  b === O && L(!0))
                                : ee(!0);
                            }
                          },
                          b === O ? "Finish" : "Next Question"
                        )
                      )
                    )
                  )
                ),
            $
              ? r.a.createElement(
                  P.a,
                  { variant: "outlined", severity: "error" },
                  "Please answer this question to move forward!"
                )
              : ""
          );
        },
        W = function(e) {
          var t = new Date(1e3 * e),
            a = t.getUTCHours(),
            n = t.getUTCMinutes(),
            r = t.getSeconds(),
            c = "";
          return (
            a && (c += a.toString().padStart(2, "0") + "hr "),
            n && (c += n.toString().padStart(2, "0") + "min "),
            r && (c += r.toString().padStart(2, "0") + "seconds "),
            c
          );
        },
        H = a(76),
        L = a.n(H),
        K = a(77),
        z = a.n(K),
        Y = function(e) {
          return r.a.createElement(
            r.a.Fragment,
            null,
            r.a.createElement(
              "a",
              {
                target: "_blank",
                href: "https://twitter.com/intent/tweet?text=".concat(e.value)
              },
              r.a.createElement("img", { style: { height: "2rem" }, src: L.a })
            ),
            "\xa0",
            r.a.createElement(
              "a",
              {
                target: "_blank",
                href: "https://api.whatsapp.com/send?text=".concat(e.value)
              },
              r.a.createElement("img", { style: { height: "2rem" }, src: z.a })
            )
          );
        },
        Z = Object(E.a)(function(e) {
          return {
            paper: {
              maxWidth: 1e3,
              margin: "".concat(e.spacing(3), "px auto"),
              padding: e.spacing(5)
            },
            headingText: { verticalAlign: "middle", color: "blue" },
            socialMediaContainer: { maxHeight: "1rem" }
          };
        }),
        U = function() {
          var e = Z(),
            t = Object(u.f)(),
            a = Object(n.useState)(0),
            c = Object(g.a)(a, 2),
            i = c[0],
            l = c[1],
            s = Object(n.useState)(0),
            o = Object(g.a)(s, 2),
            m = (o[0], o[1]),
            E = Object(n.useState)(0),
            p = Object(g.a)(E, 2),
            d = p[0],
            b = p[1],
            f = Object(n.useState)(0),
            h = Object(g.a)(f, 2),
            O = h[0],
            y = h[1],
            w = Object(n.useState)(0),
            x = Object(g.a)(w, 2),
            P = x[0],
            C = x[1];
          return (
            Object(n.useEffect)(function() {
              var e = JSON.parse(
                localStorage.getItem("currentResults") || "{}"
              );
              l(e.timer || 0),
                m(e.totalQuestions),
                b(e.correctAnswers),
                y(e.wrongAnswers),
                C(e.percentageSuccess);
            }, []),
            r.a.createElement(
              "div",
              null,
              r.a.createElement(
                S.a,
                { className: e.paper },
                r.a.createElement(
                  v.a,
                  {
                    spacing: 2,
                    container: !0,
                    xs: 12,
                    item: !0,
                    justify: "center"
                  },
                  r.a.createElement(
                    v.a,
                    { container: !0, xs: 12, item: !0, justify: "center" },
                    r.a.createElement(
                      j.a,
                      {
                        variant: "h4",
                        display: "inline",
                        className: e.headingText
                      },
                      "Congratulations!!!"
                    )
                  ),
                  r.a.createElement(
                    v.a,
                    { container: !0, xs: 12, item: !0, justify: "center" },
                    "You have completed the test in " + W(i).toString()
                  ),
                  r.a.createElement(
                    v.a,
                    { container: !0, xs: 12, item: !0, justify: "center" },
                    "Correct Answers: " +
                      d.toString() +
                      ", Wrong Answers: " +
                      O.toString()
                  ),
                  r.a.createElement(
                    v.a,
                    { container: !0, xs: 12, item: !0, justify: "center" },
                    "Overall Performance: " + P.toString() + "%"
                  ),
                  r.a.createElement(
                    v.a,
                    { container: !0, xs: 12, item: !0, justify: "center" },
                    r.a.createElement(
                      A.a,
                      {
                        color: "primary",
                        onClick: function() {
                          return t.push("/allresults");
                        }
                      },
                      "View all results"
                    )
                  ),
                  r.a.createElement(
                    v.a,
                    { container: !0, xs: 12, item: !0, justify: "center" },
                    r.a.createElement(
                      A.a,
                      {
                        color: "primary",
                        onClick: function() {
                          return t.push("/");
                        }
                      },
                      "Take the test again"
                    )
                  ),
                  r.a.createElement(
                    v.a,
                    {
                      container: !0,
                      xs: 12,
                      item: !0,
                      justify: "center",
                      className: e.socialMediaContainer
                    },
                    r.a.createElement(j.a, null, "Share the results \xa0"),
                    r.a.createElement(Y, {
                      value:
                        "Scored " +
                        P.toString() +
                        "percentage in teikametrics quiz"
                    })
                  )
                )
              )
            )
          );
        },
        _ = a(176),
        $ = a(180),
        ee = a(179),
        te = a(175),
        ae = a(177),
        ne = a(178),
        re = Object(E.a)({
          root: { width: "100%" },
          container: { maxHeight: 500 }
        }),
        ce = function() {
          var e = re(),
            t = Object(u.f)(),
            a = Object(n.useState)([]),
            c = Object(g.a)(a, 2),
            i = c[0],
            l = c[1];
          return (
            Object(n.useEffect)(function() {
              l(JSON.parse(localStorage.getItem("resultsHistory") || "[]"));
            }, []),
            r.a.createElement(
              r.a.Fragment,
              null,
              r.a.createElement(
                A.a,
                {
                  color: "primary",
                  onClick: function() {
                    return t.push("/");
                  }
                },
                "Take the test again"
              ),
              r.a.createElement(
                S.a,
                { className: e.root },
                r.a.createElement(
                  te.a,
                  { className: e.container },
                  r.a.createElement(
                    _.a,
                    { "aria-label": "simple table" },
                    r.a.createElement(
                      ae.a,
                      null,
                      r.a.createElement(
                        ne.a,
                        null,
                        r.a.createElement(ee.a, null, "Correct Answers"),
                        r.a.createElement(ee.a, null, "Wrong Answers"),
                        r.a.createElement(ee.a, null, "Performance"),
                        r.a.createElement(ee.a, null, "Time Taken")
                      )
                    ),
                    r.a.createElement(
                      $.a,
                      null,
                      i.map(function(e) {
                        return r.a.createElement(
                          ne.a,
                          { key: e.timer },
                          r.a.createElement(ee.a, null, e.correctAnswers),
                          r.a.createElement(ee.a, null, e.wrongAnswers),
                          r.a.createElement(ee.a, null, e.percentageSuccess),
                          r.a.createElement(ee.a, null, W(e.timer))
                        );
                      })
                    )
                  )
                )
              )
            )
          );
        },
        ie = function() {
          return r.a.createElement(
            "div",
            { className: "Quiz" },
            r.a.createElement(
              l.a,
              { theme: o },
              r.a.createElement(
                m.a,
                null,
                r.a.createElement(
                  u.c,
                  null,
                  r.a.createElement(u.a, { path: "/test", component: X }),
                  r.a.createElement(u.a, { path: "/finish", component: U }),
                  r.a.createElement(u.a, {
                    path: "/allresults",
                    component: ce
                  }),
                  r.a.createElement(u.a, { path: "/", component: x })
                )
              )
            )
          );
        };
      i.a.render(r.a.createElement(ie, null), document.getElementById("root"));
    },
    76: function(e, t) {
      e.exports =
        "data:image/png;base64,";
    },
    77: function(e, t) {
      e.exports =
        "data:image/png;base64,";
    },
    92: function(e, t, a) {
      e.exports = a(120);
    },
    97: function(e, t, a) {}
  },
  [[92, 1, 2]]
]);
//# sourceMappingURL=main.aa42dac8.chunk.js.map
